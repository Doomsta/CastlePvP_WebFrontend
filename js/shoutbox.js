...
/* Fetch from Database */
function fetch(){ $.ajax({ url:"lib/shoutbox_json.php", context:document.body, success:function(msg){ $("#entries").html(msg); }}); }
/* Write to Database */
function write($n, $m){
	$.ajax({ type:"POST", url:"lib/shoutbox_json.php", data:"action=write&name="+$n+"&message="+$m,success:function(msg){ fetch(); } });
}
